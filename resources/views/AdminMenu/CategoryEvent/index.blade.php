<div class="col-xl-12">
    <div id="panel-1" class="panel">
        <div class="panel-hdr">
            <h2>
                Category Event<span class="fw-300"><i>List</i></span>
            </h2>
...
@include('AdminMenu.CategoryEvent.form')
<script>
    {
        var idEdit = null;
        ...
        function update() {
            if (0 === 0) {
                $.ajax({
                    url: "{{ url('/api/admin/category-event') }}/"+idEdit,
                    type: "PUT",
                    data: valueFil(),
                    beforeSend: function(xhr) {
                        blockagePage('Please Wait...');
        ...
        function destroy(id) {
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: "{{ url('/api/admin/category-event') }}/"+id,
                        type: "DELETE",
                        beforeSend: function(xhr) {
                            blockagePage('Please Wait...');
                            xhr.setRequestHeader('Authorization', 'Bearer ' + AuthToken);
